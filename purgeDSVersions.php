...
//NB: we start our loop at 1 because we are skipping the first element 
// in the array of DS's because it is the oldest and we are using that 
// as our comparison base
for ($i = 1; $i <= count($oldestToNewestDS) - 1; $i++) {
    
    $currentDS = $oldestToNewestDS[$i];
    drush_print('Comparing checksum of '.$currentDS['dsVersionID'].' to oldest DS');
    
    $currentChecksum = $currentDS['dsChecksum'];
//     $nextChecksum    = $oldestToNewestDS[$i+1]['dsChecksum'];
    
    if ($currentChecksum === $oldestDS['dsChecksum']) {
        drush_print("Checksums are the same, continuing");
        continue;
    }
    else {
        $allTheSame = FALSE;
        // delete checksums from the original up to this one
        $startDT = createCustomDT($oldestDS, '-1 second');
        $endDT = createCustomDT($oldestToNewestDS[$i], '-1 second');
        drush_print("Checksums are different! Stopping here.");
        
        $api_m->purgeDatastream($pid, $dslabel, array(
        	'startDT' => createCustomDT($oldestDS, "+1 second"),
          'endDT' => createCustomDT($currentDS, "-1 second"),
          'logMessage' => '',
        ));
...
if ($allTheSame) {
    ...
    drush_print("Checksums are all the same!");
    drush_print("Deleting all checksums except the oldest...");
    $api_m->purgeDatastream($pid, $dslabel, array(
    	'startDT' => createCustomDT($oldestDS, "+1 second"),
      'endDT' => NULL,
      'logMessage' => '',
